...
#define PW_DEBUG 2  // [0, 1, 2]

#if defined(PW_DEBUG) && PW_DEBUG == 1
 #define PW_DEBUG_1(fmt, args...) fprintf(stderr, "PW_DEBUG: %s(): " fmt, __func__, ##args)
#elif defined(PW_DEBUG) && PW_DEBUG == 2
 #define PW_DEBUG_1(fmt, args...) fprintf(stderr, "PW_DEBUG: %s(): " fmt, __func__, ##args)
 #define PW_DEBUG_2(fmt, args...) fprintf(stderr, "PW_DEBUG: %s(): " fmt, __func__, ##args)
#else
 #define PW_DEBUG_1(fmt, args...) /* Don't do anything in release builds */
 #define PW_DEBUG_2(fmt, args...) /* Don't do anything in release builds */
#endif
...
rm_purged(struct threadData *cur, char* DirName, time_t purgedir_atime, int purgedir_fd)
{
    ...
    *s++ = '/';
    ...
    PW_DEBUG_1("%s number of files: %ld\n", DirName, fcount);
    closedir(purgeDIR);
    return fcount;
}
...
*fileDir( void *arg )
{
    ...
    PW_DEBUG_2("threadID=%ld,rdepth=%ld,file=%s\n", cur->THRDid, cur->depth, cur->dname);
        ...
        *t = '\0';
        ...
        if ( S_ISDIR(f.st_mode) ) {
            ...
            if ( thrd_ptr->THRDid != cur->THRDid ) {  /* new thread available */
                PW_DEBUG_1("creating new thread: %s\n", thrd_ptr->dname);
...
return_thread:
    if ( cur->flag == 0 ) { /* this instance of fileDir is a thread */
        pthread_mutex_lock ( &mutexFD );
        PW_DEBUG_2("msg=endTHRD,threadID=%ld,rdepth=%d,file=<%s>\n", cur->THRDid, cur->flag, cur->dname);
        --ThreadCNT;
        cur->THRDid = -1;
        pthread_mutex_unlock ( &mutexFD );
